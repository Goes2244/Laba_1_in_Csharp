...
{
    // Метод для безопасного чтения целых чисел
    static int ReadInt(string prompt)
    {
        ...
        while (true) // Бесконечный цикл пока не получим корректный ввод
        {
            ...
            if (int.TryParse(Console.ReadLine(), out result))
                return result; // Успешный ввод
            Console.WriteLine("Ошибка! Введите целое число.");
        }
    }

    // Метод для безопасного чтения символов
    ...
    {
        Console.Write(prompt);
        return Console.ReadLine() ?? ""; // Защита от null
    }

    // Метод для чтения массива целых чисел
    static int[] ReadIntArray(string sizePrompt, string elementPrompt)
    {
        int size = ReadInt(sizePrompt); // Читаем размер массива
    ...
    {
        // Создаем экземпляры классов для работы
        Lab1 lab1 = new Lab1();
        Lab2 lab2 = new Lab2();
        Lab4 lab4 = new Lab4();
